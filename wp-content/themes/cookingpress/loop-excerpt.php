 ...
 */
?>
<section id="content" class="exclay" role="main" >
    ...
    <?php while (have_posts()) : the_post(); ?>
        ...
        <?php printf( '<a href="%1$s" class="published-time" title="%2$s" rel="bookmark"><span class="entry-date">%3$s</span></a>',get_permalink(),esc_attr( get_the_time() ),get_the_date()); ?>
         <h2 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php printf(esc_attr__('Permalink to %s', 'purepress'), the_title_attribute('echo=0')); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
             <div class="entry-content">
                        <?php 
                        if (has_post_thumbnail()) { $words_limit = 18;} else { $words_limit = 50; }
                            if (post_password_required()) : ?>
                            <?php echo limit_words(get_the_excerpt('Read more &raquo;'), $words_limit);   echo "<a class='read-more' href=".get_permalink().">"; _e(' read more', 'purepress'); echo " &rarr;</a>"; ?>
                        <?php else : ?>
                            <?php echo limit_words(get_the_excerpt('Read more &raquo;'), $words_limit);   echo "<a class='read-more' href=".get_permalink().">"; _e(' read more', 'purepress'); echo " &rarr;</a>";  endif; ?>
